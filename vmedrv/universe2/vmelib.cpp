...
//----------------------------------------------------------------------------
void* VMEBridge::getAddr(int handle, int size)
{
    void *mapped_data;

    mapped_data = (void *) mmap(NULL, size, PROT_WRITE | PROT_READ, MAP_SHARED,
                                handle, 0);

    if (mapped_data == (void *) -1) {
...
//----------------------------------------------------------------------------
void* VMEBridge::requestDMA(int nrOfBufs)
{
    int result, i = 0;
...
void* VMEBridge::requestDMA(void)
{
    return requestDMA(1);
}
...
//----------------------------------------------------------------------------
void* VMEBridge::getPciBaseAddr(int image)
{
    if ((image < 0) || (image > 17)) {
