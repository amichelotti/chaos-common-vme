...
vmewrap_vme_handle_t vmewrap_init_driver(vme_driver_t driver, void *params)
{
	...
	if (p == NULL)
	{
		DERR("cannot allocate resources");
		return 0;
	}
	...
	case VME_UNIVERSE2_DRIVER:
		...
		{
			DERR("cannot initialize universe2");
			delete p;
			return 0;
		}
		break;
	...
	case VME_CAEN2718_DRIVER:
		...
		{
			DERR("cannot initialize caenvme");
			delete p;
			return 0;
		}
		break;
	case VME_LINUX_USER_DRIVER:
		...
		{
			DERR("cannot initialize uservme");
			delete p;
			return 0;
		}
		break;
	case VME_SIS3153_ETH_DRIVER:
		...
		{
			DERR("cannot initialize sis3153_eth");
			delete p;
			return 0;
		}
		break;
...
int vmewrap_deinit_driver(vmewrap_vme_handle_t p)
{
	...
	case VME_UNIVERSE_DRIVER:
		...
		{
			DERR("cannot deinitialize universe2");
	...
	case VME_CAEN2718_DRIVER:
		...
		{
			DERR("cannot deinitialize caenvme");
			delete p;
			return -1;
		}
	...
	case VME_SIS3153_ETH_DRIVER:
		...
		{
			DERR("cannot deinitialize sis3153");
			delete p;
			return -1;
		}
		break;
	case VME_LINUX_USER_DRIVER:
		...
		{
			DERR("cannot deinitialize uservme");
			delete p;
			return -1;
		}
		break;
...
vmewrap_window_t vmewrap_vme_open_master(vmewrap_vme_handle_t handle, uint32_t master_add, uint32_t master_size, vme_addressing master_addressing, vme_access dw, vme_opt vme_opts)
{
	...
	if (master_size == 0)
	{
		DERR("size must be greater than 0");
		return 0;
	}

	if (handle->vme_init(handle))
	{
		DERR("cannot initialize vme");
		perror("vme_init");
		return 0;
	}
	...
	{
		DERR("too many open vme windows %d", handle->nwindow);
		return 0;
	}
...
int vmewrap_vme_close(vmewrap_window_t handle)
{
	...
	if (handle)
	{
		...
		if (handle->parent->nwindow > 0)
			handle->parent->nwindow--;
		delete (handle);
		return 0;
	}
	return -4;
