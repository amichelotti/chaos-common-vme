...
caen792_handle_t caen792_open(uint32_t address ){
  ...
  DPRINT("caen792 successfully mapped at @0x%x\n",mapped_address);
  ...
  if(boardid!=BOARD_ID || manufactureid!=MANUFACTURE_ID){
    ...
    caen792_close((caen792_handle_t)p);
    return 0;
  }
  
  return (caen792_handle_t) p; 
}
...
int32_t caen792_close(caen792_handle_t h){
  ...
  if(handle){
    int ret;
    DPRINT("caen792 sucessfully closed\n");
...
static int acquire_event_channels(caen792_handle_t h,uint32_t *lowres,uint32_t*hires,int start_chan,int max_chan){
  _caen_qdc_handle_t* handle = h;
  int cnt=0;
  evt_buffer_t a;
...
int32_t caen792_acquire_channels_poll(caen792_handle_t h,uint32_t *lowres,uint32_t*hires,int start_channel,int nchannels,uint64_t *cycle,int timeo_ms){
  int ret = 0;
  _caen_qdc_handle_t* handle = h;
  uint32_t status;
  uint32_t counter;
  ...
  //
  do{
    // get the status from the caen792
  ...
  } while(((status&CAEN792_STATUS_DREADY)==0)&&((diff)<=(timeo_ms*1000)));
  ...
  if(status&CAEN792_STATUS_DREADY){
...
caen792_handle_t caen792_LV_open(uint32_t mapped_address,errorStruct *error ){
  ...
  p->mapped_address = mapped_address;
  p->cycle = 0;
  DPRINT("setting caen792 Linux address to @0x%x\n",mapped_address);
...
int32_t caen792_LV_acquire_channels_poll(caen792_handle_t h,void *lowres,void*hires,int32_t* event_under_run,int timeo_ms,errorStruct*error){
  ...
  *event_under_run =-1;
  if((*lv_lowres)->arg && (*lv_hires)->arg){
    ...
    ret =caen792_acquire_channels_poll(h,(uint32_t *)(*lv_lowres)->arg,(uint32_t*)(*lv_hires)->arg,0,NCHANNELS,&counter,timeo_ms); 
